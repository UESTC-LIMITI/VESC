...
// Threads
static THD_FUNCTION(shoot_thread, arg);
static THD_WORKING_AREA(shoot_thread_wa, 512);
...
void app_custom_start(void) {
	...
	stop_now = false;
	chThdCreateStatic(shoot_thread_wa, sizeof(shoot_thread_wa),
			NORMALPRIO, shoot_thread, NULL);
...
static void pwm_callback(void) {
	mc_interface_shoot_excute(mc_interface_get_shoot_parameter());
