...
void spi_bb_transfer_8(
	...
	for (int i = 0; i < length; i++) {
		...
		for (int bit = 0; bit < 8; bit++) {
			if(s->mosi_gpio) {
				GPIO_SET(s->mosi_gpio, s->mosi_pin, send >> 7);
				send <<= 1;
			}

			GPIO_SET(s->sck_gpio, s->sck_pin, 1);
			spi_bb_delay();

			int samples = 0;
			samples += GPIO_GET(s->miso_gpio, s->miso_pin);
			__NOP();
			samples += GPIO_GET(s->miso_gpio, s->miso_pin);
			__NOP();
			samples += GPIO_GET(s->miso_gpio, s->miso_pin);
			...
			GPIO_SET(s->sck_gpio, s->sck_pin, 0);
...
void spi_bb_transfer_16(
	...
	for (int i = 0; i < length; i++) {
		...
		for (int bit = 0; bit < 16; bit++) {
			if(s->mosi_gpio) {
				GPIO_SET(s->mosi_gpio, s->mosi_pin, send >> 15);
				send <<= 1;
			}
...
void ssc_bb_transfer_16(
	...
	// 		// read when clk low
	// 		receive <<= 1;
	// 		receive |= GPIO_GET(s->mosi_gpio, s->mosi_pin);
	// 	}
...
void spi_bb_begin(spi_bb_state *s) {
	spi_bb_delay();
	GPIO_SET(s->nss_gpio, s->nss_pin, 0);
	spi_bb_delay();
}

void spi_bb_end(spi_bb_state *s) {
	spi_bb_delay();
	GPIO_SET(s->nss_gpio, s->nss_pin, 1);
	spi_bb_delay();
}
