...
spi_bb_state encoder_spi = {
	.miso_gpio = ENC_3_GPIO_Port,
	.miso_pin = ENC_3_Pin,
	.sck_gpio = ENC_1_GPIO_Port,
	.sck_pin = ENC_1_Pin,
	.mosi_gpio = NULL,
	.mosi_pin = 0,
	.nss_gpio = ENC_2_GPIO_Port,
	.nss_pin = ENC_2_Pin
};

AS504x_config_t as5047_cfg = {0};
AS504x_state as5047_state = {0};
AS504x_diag as5047_diag = {0};
...
void enc_as504x_routine(AS504x_config_t *cfg) {   //AS5047的SPI通信整个流程  纯纯SPI通信
	...
	if (spi_bb_check_parity(pos) && !cfg->state.spi_data_err_raised) {
		pos &= 0x3FFF;
		cfg->state.last_enc_angle = ((float) pos * 360.0) / 16384.0;  //输出的是角度值
